...
export default function PersonalizeScreen(){const {progress,save,busy}=useOnboarding();const [starting,setStarting]=useState(false);const [error,setError]=useState<string|null>(null);const selected=progress.focus??[];
 ...
 return <FlowScreen><Text style={styles.eyebrow}>A little intention</Text><Text style={styles.title}>What feels most{'\n'}missing lately?</Text><Text style={styles.body}>Choose up to three. Just for you.</Text>{choices.map(([focus,title])=><Choice key={focus} title={title} selected={selected.includes(focus)} disabled={busy} onPress={()=>{void toggle(focus);}}/>)}{error&&<Notice>{error}</Notice>}<Button label="Continue" busy={starting} disabled={busy} onPress={()=>{void start();}}/><Button label="Back to your path" secondary disabled={busy} onPress={()=>{void save({step:'path'});}}/></FlowScreen>;
}