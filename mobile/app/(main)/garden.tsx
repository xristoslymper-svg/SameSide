...
export default function Garden(){
 ...
 const flower=findFlower(state.data?.program.selectedFlower??null);const stage=sharedGrowth(state.data?.garden.stage_key??'seed',state.data?.program.day??1);const paired=state.data?.program.memberCount===2;
 return <Screen compact><View style={{marginBottom:18}}><AppHeader/></View><View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap'}}><Text style={styles.eyebrow}>Your garden</Text><Text style={[styles.small,{textTransform:'uppercase',letterSpacing:.8}]}>The Routine{state.data?` · Week ${state.data.program.week}`:''}</Text></View>
  ...
  <FlowerStory flower={story} close={()=>setStory(null)}/><RealGarden visible={learn} close={()=>setLearn(false)}/><CommunityGarden visible={communityOpen} close={()=>setCommunityOpen(false)}/>
 </Screen>;
}
