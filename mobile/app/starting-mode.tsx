...
export default function StartingModeScreen() {
  ...
  return <FlowScreen><Text style={styles.eyebrow}>Your first step</Text><Text style={styles.title}>How would you{"\n"}like to begin?</Text>
    ...
    <Choice title="Start together" description="Make the first move. Your partner can join when they’re ready." selected={progress.intent === 'together'} disabled={busy} onPress={() => { void save({ intent: 'together' }); }}/>
    <Choice title="Start on my own" description="A small change can begin with you. Invite your partner later, if you’d like." selected={progress.intent === 'solo'} disabled={busy} onPress={() => { void save({ intent: 'solo' }); }}/>
