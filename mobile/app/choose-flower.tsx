...
function FlowerSelection() {
 ...
 return <Screen><Brand/>{legacy && <Button label="Back to Garden" secondary disabled={busy} onPress={() => router.dismissTo('/garden')}/>}<LoadState {...state}/>
  {!state.loading && state.data && (selected ? <><Text style={styles.eyebrow}>{chosen ? 'You chose' : 'Your shared'} {selected.name}</Text><Text style={styles.title}>{selected.meaning}</Text><BotanicalFlower flower={selected.id}/><Text style={styles.body}>This is the flower you’ll grow together.</Text>{!legacy&&<View style={{marginTop:12,padding:16,borderRadius:20,backgroundColor:'#F4EFE7'}}><Text style={styles.eyebrow}>FROM DIGITAL TO REAL</Text><Text style={styles.cardTitle}>When it blooms, we’ll plant the same species in the real Same Side Garden.</Text></View>}<Button label={startLabel} busy={busy} onPress={() => { void start(); }}/></>
  ...
  : <><Text style={styles.eyebrow}>Choose what you’ll grow</Text><Text style={styles.title}>Every flower carries a story.</Text><Text style={styles.body}>Choose the one whose meaning feels like yours.</Text><View style={{marginTop:8,marginBottom:18,paddingVertical:13,paddingHorizontal:15,borderRadius:18,backgroundColor:'#F4EFE7'}}><Text style={styles.small}>When your journey reaches bloom, we’ll plant the same species in the real Same Side Garden.</Text></View>
